		...
		states: {
			...
			connecting: {
				_onEnter: function() {
					...
					self.conn.on('error', function onError( e ) {
						self.emit( 'error', e );
					});
				},
				...
				_onEnter : function() {
					...
					_.each(self.config.queues, function(qConfig, name) {
						self.conn.queue(name, qConfig.options, function(queue) {
							...
							subscriptionOptions = qConfig.subscriptionOptions || {};
							queue.subscribe( subscriptionOptions, function(message, headers, envelope) {
